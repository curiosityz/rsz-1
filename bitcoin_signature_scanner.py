...
class BitcoinSignatureScanner:
    ...
    def __init__(self, chainstack_url: str, api_key: Optional[str] = None,
                 db_path: str = "signatures.db", batch_size: int = 100):
    ...
    def scan_new_blocks(self):
        ...
        try:
            ...
            last_analyzed = self.db.get_last_analyzed_block() or 0
            
            if latest_block <= last_analyzed:
                print(f"No new blocks to analyze. Last analyzed: {last_analyzed}")
                return
            
            print(f"Scanning blocks {last_analyzed + 1} to {latest_block}...")
            start_time = time.time()
            
            # Analyze in batches
            for batch_start in range(last_analyzed + 1, latest_block + 1, self.batch_size):
                batch_end = min(batch_start + self.batch_size - 1, latest_block)
                
                print(f"Analyzing batch: blocks {batch_start}-{batch_end}")
                ...
                print(f"Batch complete: {result.signatures_extracted} signatures analyzed")
    ...
    def run_continuous_scan(self, interval_seconds: int = 600):
        ...
        print(f"Starting continuous scan with {interval_seconds}s interval")
...
def main():
    ...
    print(f"Starting continuous scan (interval: {SCAN_INTERVAL}s)")
